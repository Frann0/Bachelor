...
const Canvas = () => {
                ...
                client.joinOrCreate<State>("my_room").then(r => {
                    ...
                    roomRef.current.onMessage("state", (state: State) => {
                        ...
                        if (mainPlayerIdRef.current) {
                            const mpI = Object.keys(state.players).findIndex(id => id === mainPlayerIdRef.current, 0);
                            mainPlayerRef.current = Object.values(state.players)[mpI];
                            console.log(mainPlayerRef.current);
                        }
                        drawPlayers(state.players);
                    });
                });
    ...
    const drawPlayers = (players: { [id: string]: Player }) => {
        ...
        const cameraOffsetX = mainPlayerRef.current ? offscreenCanvas.width / 2 - mainPlayerRef.current.x : 0;
        const cameraOffsetY = mainPlayerRef.current ? offscreenCanvas.height / 2 - mainPlayerRef.current.y : 0;

        // Draw players on the off-screen canvas with camera offset
        for (let id in players) {
            const player = players[id];
            offscreenContext.fillRect(player.x + cameraOffsetX, player.y + cameraOffsetY, 10, 10);
            offscreenContext.fillText(player.name, player.x + cameraOffsetX, player.y + cameraOffsetY - 10);
        }
